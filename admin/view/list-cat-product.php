...
function listCatTable($items,$space='', $parent = 0)
{
    ...
    foreach($items as $item)
    {
        if ($item['parent_id'] == $parent) {
			...
			$table_html .= ' <form action="control/proccess-cat-product.php" method="post">
                <td class="numeric">';
			$table_html .= '<a href="index.php?view=form-cat-product&act=update&id='. $item['cat_id'].'" class="btn default btn-xs purple"><i class="fa fa-edit"></i>
                Sửa</a></td>';
